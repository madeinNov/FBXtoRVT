...
namespace FBXtoRVT.Core
{
    ...
    ///     이 패밀리는 Primary 커넥터가 "상" 쪽이므로, 먼 쪽이 Primary 면 'ADAPTOR_상', 아니면 'ADAPTOR_하'.
    ///
    ...
    public static class ElbowAdapterHelper
    {
        ...
        // 대상 엘보 조립품 / 기준 장비의 패밀리명 키워드
        private const string ElbowAdptFamilyKeyword = "ASSEMBLY_ELBOW_ADPT_LOT-FLON";
        private const string ScrubberFamilyKeyword = "SCRUBBER";

        // 켤 YES/NO 인스턴스 파라미터 이름 (ADAPTOR 는 밑줄인 점에 주의)
        private const string ParamAdaptorUpper = "ADAPTOR_상";
        private const string ParamAdaptorLower = "ADAPTOR_하";
        ...
        public static RunResult Run(Document doc, View view)
        {
            ...
            foreach (FamilyInstance fi in ElementUtils.CollectFamilyInstances(doc, view, ElbowAdptFamilyKeyword))
            {
                elbowIds.Add(fi.Id);
            }
            result.ElbowCount = elbowIds.Count;

            // SCR 장비도 여기서 한 번만 모은다.
            ...
            List<ScrubberRef> scrubbers, RunResult result)
        {
            ...
            if (conns.Count != 2)
            {
                LogUtils.Log($"엘보(Id={elbowId}) End 커넥터가 {conns.Count}개(2개 아님) -> 건너뜀.");
            ...
            {
                LogUtils.Log($"엘보(Id={elbowId}) 열린 커넥터가 있어 건너뜀.");
            ...
            if (elbowCenter == null)
            {
                LogUtils.Log($"엘보(Id={elbowId}) 중심점을 구하지 못해 건너뜀.");
                result.SkippedNotTwoCount++;
                return;
            }
            ...
            {
                LogUtils.Log($"엘보(Id={elbowId}) 먼 쪽 커넥터(Id={farConn.Id})가 배관이 아님 -> 건너뜀. " +
                    $"기준장비 Id={scrubber.Id} 거리={dist0:F3}/{dist1:F3}ft");
                result.SkippedNotPipeCount++;
                return;
            }

            // 5) 그 커넥터 쪽 ADAPTOR 파라미터를 켠다. (이 패밀리는 Primary 가 "상" 쪽)
            bool isPrimary = ElementUtils.IsPrimaryConnector(farConn);
            string paramName = isPrimary ? ParamAdaptorUpper : ParamAdaptorLower;
            ...
            LogUtils.Log($"엘보(Id={elbowId}) 기준장비 Id={scrubber.Id} " +
                $"먼쪽 커넥터 Id={farConn.Id} Primary={isPrimary} -> '{paramName}' 체크");
