...
namespace FBXtoRVT.Core
{
    ...
    ///
    /// [짝짓기]
    /// 유저가 1차/2차로 여러 배관을 선택하면, 직각 + 꼬인 위치 조건을 만족하는 조합 중
    /// 서로 가장 가까운 쌍부터 1:1 로 자동 매칭한다.
    /// 다만 한쪽 선택이 배관 1개뿐이면, 그 배관은 여러 번 재사용한다. (1 : N 매칭)
    ///
    /// 새로 만든 배관의 배관 타입 / System Type / 지름은 1차로 선택한 배관을 따라간다.
    /// </summary>
    public static class RightAnglePipeHelper
    {
        ...
        {
            public PipeAxis Base;       // 1차 선택 배관 (타입/지름/System Type 기준)
            public PipeAxis Target;     // 2차 선택 배관
            public XYZ BaseFoot;        // 공통수선의 발 (첫 배관 직선 위)
            public XYZ TargetFoot;      // 공통수선의 발 (둘째 배관 직선 위)
        ...
        public class RunResult
        {
            ...
            /// <summary>직선이 아니라 제외한 배관 수(1차 + 2차)</summary>
            public int NotStraightCount;

            /// <summary>이미 직각으로 만나서 사잇배관이 필요 없는 기준 배관 수</summary>
            public int AlreadyIntersectCount;

            /// <summary>직각인 상대 배관을 못 찾은 기준 배관 수</summary>
            public int NoPerpendicularCount;

            /// <summary>직각 상대는 있었지만 그 배관이 다른 쌍에 먼저 배정된 기준 배관 수</summary>
        ...
        /// <summary>
        /// 1차 선택 배관들과 2차 선택 배관들을 짝지어 사잇배관을 만든다.
            ...
            List<PipeAxis> baseAxes, List<PipeAxis> targetAxes, double minLength)
        {
            ...
            foreach (PipeAxis b in baseAxes)
            {
                ...
                {
                    if (b.Pipe.Id == t.Pipe.Id) continue;       // 같은 배관끼리는 제외
                    ...
                    if (foot1.DistanceTo(foot2) < minLength) continue;   // 이미 만남 → 사잇배관 불필요
                    ...
                    candidates.Add(new PairCandidate
                    {
                        ...
                        BaseFoot = foot1,
                        TargetFoot = foot2,
                        Nearness = SegmentDistance(b.Line, t.Line)
                    });
                }
            }
