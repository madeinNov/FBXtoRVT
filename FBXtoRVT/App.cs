    ...
    public class App : IExternalApplication
    {
        ...
        private void CreateCommonConnectPanel(UIControlledApplication application, string assemblyPath)
        {
            ...
            AddButton(panel, assemblyPath,
                ...
                "열린 커넥터가 정확히 1개 들어있으면 그 커넥터를 대상으로 인식합니다. FLANGE 는 열린 커넥터가 " +
                "2개이거나 1개(Primary)면 'FLANGE 하' 체크박스를 해제하고 Primary 를 장비에 붙이고, " +
                "1개(Primary 아님)면 'FLANGE 상' 을 해제하고 그 커넥터를 장비에 붙입니다. (BELLOWS 는 상/하 반대) " +
        ...
        private void CreateCommonPipePanel(UIControlledApplication application, string assemblyPath)
        {
            ...
            AddButton(panel, assemblyPath,
                "ReducerButton",
                "Reducer\n생성기",
                "FBXtoRVT.Commands.ReducerCommand",
                "배관을 클릭하고 지름(mm)을 입력하면, 열린 커넥터에 오토라우팅 리듀서를 넣습니다.",
                "배관을 클릭하면 숫자 입력창(기본값 50)이 뜨고, 클릭한 지점에 가까운 열린 커넥터 " +
                "바깥쪽에 리듀서가 들어갑니다. 배관쪽 지름은 클릭한 배관의 Nominal Diameter 를 " +
                "따르고, 반대쪽 지름은 입력한 값(mm)이 됩니다. 리듀서는 배관 타입의 " +
                "Routing Preferences 에 등록된 Transition 패밀리(= 오토라우팅)를 씁니다. " +
                "리듀서만 단독으로 만들 수는 없으므로, 반대쪽에는 입력한 지름의 배관이 100mm " +
                "길이로 함께 만들어집니다. 클릭한 배관의 길이는 변하지 않고, 리듀서는 커넥터 " +
