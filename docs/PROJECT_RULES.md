...
| `공용` | 공정과 상관없이 쓰는 기능 | 직각 배관 생성기 / ELBOW&배관/플랜지 / HOPPER&플랜지 |
| `기타` | 보조(뷰 조작) 기능 | 선택 Section Box |
| `응원` | 응원 버튼 | (이름별 8개) |

**`기타` 패널은 항상 `응원` 바로 앞(= 기능 패널 중 가장 오른쪽)에 둡니다.**

### 이유

버튼이 계속 늘어나므로, **어느 공정에서 쓰는 기능인지**로 먼저 묶어야
사용자가 찾기 쉽습니다.

`선택 Section Box` 같은 보조 기능은 다른 기능과 성격이 다르므로 `기타` 로 빼고
위치를 고정해 두면, 기능이 늘어나도 버튼을 찾는 위치가 바뀌지 않습니다.
...
- [ ] 성격에 맞는 패널에 버튼을 추가했는가? `기타` 는 계속 가장 오른쪽인가? (규칙 2)
